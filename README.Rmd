...
The following example generates keys, runs CFAs and an EFA using these keys, computes correlations between CFA latent variables, and uses outputs from these to run ESEMs.
...
The fitted CFA models can also be used to calculate latent variable correlations. In this example, Burt's 2-stage procedure is used by setting `nagy = FALSE` to save time, but in many cases Nagy and colleagues' (2017) method will be superior. See `?sem.cor()` for further details.
