...
#' factor analysis factors in separate models for each CFA factor.
#' The function uses outputs from `cfa.from.keys()` and `efa.from.keys()` and
#' the same keys as those used in those upstream functions, so cannot be run
#' without first using those upstream functions.
#'
#' @param efa_name
